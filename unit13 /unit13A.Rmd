...
```{r}
set.seed(88)
split = sample.split(D$PoorCare, SplitRatio = 0.75)  # split vector前面放目標變數，後面放切的比率
...
#prop.table()裡面沒放參數的話就是算整個矩陣的（四格的百分比）百分比
#1橫向加起來是1
#2直向加起來是1
```
...
```{r}
exp(0.0821)
#officevists每增加1，y=1的勝率就會增加8.56%
...
&emsp; &emsp; ■ 我們需先推算原先的機率，才能推算變數的機率效果（通常會先設系數的中位數或平均值）<br><br>
</p class="wwl"><br>
...
+ 當`OfficeVisits`和`Narcotic`分別等於他們的第一分位(`Q1`)時`PoorCare = 1`的機率是11.192%
```{r}
df1 = data.frame(OfficeVisits = 7, Narcotics=0)
predict(glm1, df1, type="response")
```
...
summary(glm1)
exp(0.0821)#officevists每增加1，y=1的勝率就會增加8.56%
exp(0.0763)#Narcotics每增加1，y=1的勝率就會增加7.93%
```

&emsp; &emsp; ■ 兩個自變數的機率效果分別是？<br>
```{r}
df2 = data.frame(OfficeVisits = 7, Narcotics=0)
x=predict(glm1, df2, type="response")

df3= data.frame(OfficeVisits = 8, Narcotics=0)
y=predict(glm1, df3, type="response")

y-x

```
+ 在OfficeVisits等於第三分位時，此時的機率效果(x增加1時，y = 1增加的機率)為0.8%
```{r}
