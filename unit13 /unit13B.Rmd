...
#glm1 = glm(PoorCare ~ .,TR[2:14], family=binomial)
summary(glm1)
```
<br><hr>
...
- 準確性(accuracy正確率):TN+TF(全部猜對)/全部<br>
- 敏感性(sensitivity/true positive rate/TPR):給定他是1的情況之下我會猜他是1的機率<br>
- 明確性(specificity/TNR)：給定他是0我猜他是0的機率（我猜他不是，他不是的機率）<br>
- 偽真率(FPR)<br>
<br>
...
data.frame(y=factor(TS$PoorCare), pred=pred2) %>% 
  ggplot(aes(x=pred, fill=y)) + 
  geom_histogram(bins=20, col='white', position="stack", alpha=0.5) +
  ggtitle(" Testing data Distribution of Predicted Probability (DPP)") +
  xlab("predicted probability")
```
...
+ 有 對testing data 的準確率、辨識率、敏感性、明確性都有提高
&emsp; 【F】 為什麼它比較準(或比較不準)呢？<br><br>
+ 因為加入了新的變數之後，變數變多了，能解釋y變異的程度也增加了，所以用這個模型就能更好的預測y的機率
