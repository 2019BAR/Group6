...
**15歲以下人口比率最低的國家**
```{r}
which.min(WHO$Under15) #數字最小是第幾位 #第86個
WHO$Country[86] 
...
nrow(Outliers) #多少筆紀錄 ncol()多少欄位
Outliers[c("Country","GNI","FertilityRate")]

Outliers[Outliers$GNI > 10000 & Outliers$FertilityRate > 2.5,
         c("Country","GNI","FertilityRate")] #上述code可打成這句
```
<br>
...
+ ex：tapply(weight, sex, mean) 看不同性別體重的平均值
```{r}
tapply(WHO$Over60, WHO$Region, mean) #依Region看超60歲人口的平均值
```
...
```{r}
# Adding the variable to the dataset 在USDA裡加新欄位
...
<span style="font-size:24px">`r "\U1F4CB"`</span>: 是高蛋白又高脂肪有712 
```{r}
table(USDA$HighSodium, USDA$HighFat)
```
<br>

<span style="font-size:24px">`r "\U1F4CB"`</span>: 缺項以外的高蛋白食物的平均含鐵量和低蛋白食物的平均含鐵量
```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE)
```
<br>

<span style="font-size:24px">`r "\U1F4CB"`</span>: 缺項以外的高碳水化合物的維他命C最大值和低碳水化合物的維他命C最大值
```{r}
...
缺項以外的高碳水化合物的維他命C的摘要和低碳水化合物的維他命C的摘要
```{r}
