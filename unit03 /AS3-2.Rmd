...
lines(ProcterGamble$Date, ProcterGamble$StockPrice,col="blue",lty=2)  #直接加在同張圖
abline(v=as.Date(c("2000-03-01")), lwd=2) #lwd 寬度
#v=直,h=橫
```
...
```{r}
which(CocaCola$Date == "1995-01-01") #找出1995是在第幾欄
...
#lty=2 will make the line dashed, lty=3 will make the line dotted, lty=4 will make the line alternate between dashes and dots, and lty=5 will make the line long-dashed
...
#sort(tapply(IBM$StockPrice,months(IBM$Date), mean)) > mean(IBM$StockPrice) 大於平均的會顯示TRUE
```
