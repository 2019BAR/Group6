...
```{r}
IBM$Date[which.max(IBM$Date)] #2009
```
2009
...
```{r}
plot(CocaCola$Date,CocaCola$StockPrice,type="l") #1973 highest, 1980 lowest
```
highest year: 1973, lowest year: 1980
...
```
ProcterGamble and Boeing decreased trend in their stock price.
...
```
February, March, April, May, January have higher stock price than average(144.4).
...
tapply(GE$StockPrice,months(GE$Date),mean) %>%sort
```
December has the stock prices lower.
