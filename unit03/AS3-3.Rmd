---
title: "AS3-3: 美國的人口統計和就業資料"
<<<<<<< HEAD
author: "第 6 組"
=======
author: "第 0 組"
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
date: "`r Sys.time()`"
output: 
  html_document:
    highlight: pygments
    theme: flatly
    css: style.css
---

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE}
# 這些程式碼設定網頁的格式，並安裝、載入一些基本的套件，請大家不要去改動<br>
rm(list=ls(all=T))
knitr::opts_chunk$set(comment = NA)
knitr::opts_knit$set(global.par = TRUE)
par(cex=0.8)
options(scipen=20, digits=4, width=90)
#
if(!require(pacman)) install.packages("pacman")
pacman::p_load(dplyr)
```
<hr>

就業統計數據是政策制定者用來衡量經濟整體實力的最重要指標之一。在美國，政府使用現有人口調查（CPS）衡量失業率，該調查每月收集來自各種美國人的人口統計和就業信息。在本練習中，我們將使用講座中審查的主題以及一些使用2013年9月版的，具有全國代表性的數據集。數據集中的觀察結果代表2013年9月CPS中實際完成調查的人員，完整數據集有385個欄位，但在本練習中，我們將使用數據集CPSData.csv版本，它具有以下欄位：

+ `PeopleInHousehold`: 受訪者家庭中的人數。
+ `Region`: 受訪者居住的人口普查區域。
+ `State`: 受訪者居住的州。
+ `MetroAreaCode`: 都會區代碼，如受訪者不住都會區，則為NA；從代碼到都會區名稱的對應在`MetroAreaCodes.csv`中提供。
+ `Age`: 受訪者的年齡，以年為單位。 80代表80-84歲的人，85代表85歲及以上的人。
+ `Married`: 受訪者的婚姻狀況。
+ `Sex`: 受訪者的性別。
+ `Education`: 受訪者獲得的最高教育程度。
+ `Race`: 受訪者的種族。
+ `Hispanic`: 受訪者是否屬於西班牙裔。
+ `CountryOfBirthcode`: 識別受訪者出生國家的代碼。從代碼到國家名稱的映射在CountryCodes.csv文件中提供。
+ `Citizenship`: 受訪者的公民身份。
+ `EmploymentStatus`: 受訪者的就業狀況。
+ `Industry`: 受訪者的就業行業（僅在受僱的情況下可用）。

- - -

<<<<<<< HEAD
=======

>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
<br><br>

###  Section-1 Loading and Summarizing the Dataset

__§ 1.1__ How many interviewees are in the dataset?
```{r}
<<<<<<< HEAD
CPS=read.csv("data/CPSData.csv")
nrow(CPS)
=======
CPSData=read.csv("data/CPSData.csv")
str(CPSData)
#ans:131302
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 1.2__ Among the interviewees with a value reported for the Industry variable, what is the most common industry of employment? Please enter the name exactly how you see it.
```{r}
<<<<<<< HEAD
sort(table(CPS$Industry)) 
#Educational and health services
=======
table(CPSData$Industry)%>%sort%>%tail(1)

#ans:15017(Educational and health services)
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 1.3__ Which state has the fewest interviewees?
```{r}
<<<<<<< HEAD
sort(table(CPS$State))
#New Mexico
=======
table(CPSData$State)%>%sort%>%head(1)
table(CPSData$State)%>%sort
#ans:1102(New Mexico) 
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

Which state has the largest number of interviewees?
```{r}
<<<<<<< HEAD
sort(table(CPS$State))
#California
=======
table(CPSData$State)%>%sort%>%tail(1)
#ans:11570(California)
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 1.4__ What proportion of interviewees are citizens of the United States?
```{r}
<<<<<<< HEAD
prop.table(table(CPS$Citizenship)) 
1-0.05781
#0.9422
=======
table(CPSData$Citizenship)%>%prop.table
0.88833+0.05387#ans:0.9422
#Q:如果資料庫裡含有其他國家的資料，那要如何將它們分類開來？

>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 1.5__ For which races are there at least 250 interviewees in the CPS dataset of Hispanic ethnicity? (Select all that apply.) 

+ American Indian
+ Asian
+ cBlack
+ Multiracial
+ Pacific Islander
+ White

```{r}
<<<<<<< HEAD
table(CPS$Race,CPS$Hispanic)
#>250: American Indian, Black, Multiracial, White
=======
table(CPSData$Hispanic,CPSData$Race)
#ans:American,Black,Multiracial,White
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

<br><br> 

- - -

###  Section-2 Evaluating Missing Values

__§ 2.1__ Which variables have at least one interviewee with a missing (NA) value? (Select all that apply.)

+ PeopleInHousehold
+ Region
+ State
+ MetroAreaCode
+ Age
+ Married
+ Sex
+ Education
+ Race
+ Hispanic
+ CountryOfBirthCode
+ Citizenship
+ EmploymentStatus
+ Industry

```{r}
<<<<<<< HEAD
summary(CPS)
#MetroAreaCode, Married, Education, EmploymentStatus, Industry
=======
summary(CPSData)
is.na(CPSData)

#看表格及summary
#凡有TRUE都為NA （不可計算）
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 2.2__  Which is the most accurate:

+ The Married variable being missing is related to the Region value for the interviewee.
+ The Married variable being missing is related to the Sex value for the interviewee.
+ The Married variable being missing is related to the Age value for the interviewee. 
+ The Married variable being missing is related to the Citizenship value for the interviewee.
+ The Married variable being missing is not related to the Region, Sex, Age, or Citizenship value for the interviewee.

```{r}
<<<<<<< HEAD
table(CPS$Region, is.na(CPS$Married))
table(CPS$Sex, is.na(CPS$Married))
table(CPS$Age, is.na(CPS$Married))
table(CPS$Citizenship, is.na(CPS$Married))
#Age
```


__§ 2.3__ How many states had all interviewees living in a non-metropolitan area (aka(also known as) they have a missing MetroAreaCode value)? For this question, treat the District of Columbia as a state (even though it is not technically a state).
```{r}
table(CPS$State, is.na(CPS$MetroAreaCode))
# 2
#Alaska, Wyoming
=======
table(CPSData$Region, is.na(CPSData$Married))
table(CPSData$Sex, is.na(CPSData$Married))
table(CPSData$Age, is.na(CPSData$Married))
table(CPSData$Citizenship, is.na(CPSData$Married))
#
#
```


__§ 2.3__ How many states had all interviewees living in a non-metropolitan area (aka they have a missing MetroAreaCode value)? For this question, treat the District of Columbia as a state (even though it is not technically a state).
```{r}
table(CPSData$State,is.na(CPSData$MetroAreaCode))

#ans:3 (District of Columbia,New Jersey,Rhode Island)

>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

How many states had all interviewees living in a metropolitan area? Again, treat the District of Columbia as a state.
```{r}
<<<<<<< HEAD
# 3
# District of Columbia, New Jersey, Rhode Island
=======
#ans:2 ( Wyoming, Alaska)
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 2.4__ Which region of the United States has the largest proportion of interviewees living in a non-metropolitan area?
```{r}
<<<<<<< HEAD
prop.table(table(CPS$Region, is.na(CPS$MetroAreaCode)),1)
# Midwest
=======
table(CPSData$Region,is.na(CPSData$MetroAreaCode)) %>% prop.table(2) %>% round(3) 
#ans:Midwest
#Midwest TRUE 為最大
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 2.5__ Which state has a proportion of interviewees living in a non-metropolitan area closest to 30%?
```{r}
<<<<<<< HEAD
sort(tapply(is.na(CPS$MetroAreaCode), CPS$State, mean))
# Wisconsin
=======
tapply(is.na(CPSData$MetroAreaCode),CPSData$State,mean)
#ans:Wisconsin
#利用全部類別的non-metropolitan area平均比
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

Which state has the largest proportion of non-metropolitan interviewees, ignoring states where all interviewees were non-metropolitan?
```{r}
<<<<<<< HEAD
# Montana
=======
(tapply(is.na(CPSData$MetroAreaCode),CPSData$State,mean)%>%sort%>%tail(3))[-3][-2]
(tapply(is.na(CPSData$MetroAreaCode),CPSData$State,mean)%>%sort%>%tail(3))
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

<br><br> 

- - -

###  Section-3 Integrating Metropolitan Area Data

__§ 3.1__ How many observations (codes for metropolitan areas) are there in MetroAreaMap?
```{r}
<<<<<<< HEAD
MetroAreaMap = read.csv("data/MetroAreaCodes.csv")
nrow(MetroAreaMap)
=======
MetroAreaCodes=read.csv("data/MetroAreaCodes.csv")
nrow(MetroAreaCodes)
#ans=271
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

How many observations (codes for countries) are there in CountryMap?
```{r}
<<<<<<< HEAD
CountryMap = read.csv("data/CountryCodes.csv")
nrow(CountryMap)
=======
CountryCodes=read.csv("data/CountryCodes.csv")
nrow(CountryCodes)
#ans=149
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 3.2__ What is the name of the variable that was added to the data frame by the merge() operation?
```{r}
<<<<<<< HEAD
CPS=read.csv("data/CPSData.csv")
CPS_new = merge(CPS, MetroAreaMap, by.x="MetroAreaCode", by.y="Code", all.x=TRUE)
#colnames(CPS)
#colnames(CPS_new)
setdiff(colnames(CPS_new),colnames(CPS))
# MetroArea
=======
CPS = merge(CPSData, MetroAreaCodes, by.x="MetroAreaCode", by.y="Code", all.x=TRUE)
summary(CPS)
str(CPS)

#ans: MetroAreaCode
#從summary 看
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

How many interviewees have a missing value for the new metropolitan area variable?
```{r}
<<<<<<< HEAD
sum(is.na(CPS_new$MetroArea))
=======
#ans= NA'S 34238
#從summary看
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 3.3__ Which of the following metropolitan areas has the largest number of interviewees?
```{r}
<<<<<<< HEAD
table(CPS_new$MetroArea) %>% sort %>% tail(10)
# Boston-Cambridge-Quincy, MA-NH
=======
sort(table(CPS$MetroArea))%>%tail(10)

#
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 3.4__ Which metropolitan area has the highest proportion of interviewees of Hispanic ethnicity?
```{r}
<<<<<<< HEAD
tapply(CPS_new$Hispanic, CPS_new$MetroArea, mean) %>% sort %>% tail
# Laredo, TX
=======
tapply(CPS$Hispanic,CPS$MetroArea,mean)%>%sort%>%tail(1)

#highest proportion 可以用 tapply的mean 做計算
#ans:Laredo,TX
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 3.5__ Determine the number of metropolitan areas in the United States from which at least 20% of interviewees are Asian.
```{r}
<<<<<<< HEAD
tapply(CPS_new$Race == "Asian", CPS_new$MetroArea, mean) %>% sort %>% tail  
# 4
# Honolulu, HI; San Francisco-Oakland-Fremont, CA; San Jose-Sunnyvale-Santa Clara, CA; and Vallejo-Fairfield, CA
=======
tapply(CPS$Race=="Asian",CPS$MetroArea,mean)%>%sort%>%tail(10)
#ans:4(CA,CA,CA,HI)
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 3.6__  Passing na.rm=TRUE to the tapply function, determine which metropolitan area has the smallest proportion of interviewees who have received no high school diploma.
```{r}
<<<<<<< HEAD
tapply(CPS_new$Education == "No high school diploma", CPS_new$MetroArea, mean, na.rm=TRUE) %>% 
  sort %>% head  
# Iowa City, IA
=======
tapply(CPS$Education == "No high school diploma", CPS$MetroArea, mean,na.rm=TRUE)%>%sort%>%head(10)

#ans:2.91% IA
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```


<br><br> 

- - -

###  Section-4 Integrating Country of Birth Data

__§ 4.1__ What is the name of the variable added to the CPS data frame by this merge operation?
```{r}
<<<<<<< HEAD
CPS4 = merge(CPS_new, CountryMap, by.x="CountryOfBirthCode", by.y="Code", all.x=TRUE)
setdiff(colnames(CPS4),colnames(CPS_new))
# Country
```

How many interviewees have a missing value for the new country of birth variable?
```{r}
sum(is.na(CPS4$Country))
=======
CPS = merge(CPS, CountryCodes, by.x="CountryOfBirthCode", by.y="Code", all.x=TRUE)
summary(CPS)

#ans:Country
#
```

How many interviewees have a missing value for the new metropolitan area variable?
```{r}
table(is.na(CPS$Country))
#ans:176
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 4.2__ Among all interviewees born outside of North America, which country was the most common place of birth?
```{r}
<<<<<<< HEAD
table(CPS4$Country) %>% sort %>% tail  
# Philippines: 839
=======
table(CPS$Country)%>%sort%>%tail(10)
#ans:839(Philippines)
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 4.3__ What proportion of the interviewees from the "New York-Northern New Jersey-Long Island, NY-NJ-PA" metropolitan area have a country of birth that is not the United States? 
```{r}
<<<<<<< HEAD
table(CPS4$MetroArea == "New York-Northern New Jersey-Long Island, NY-NJ-PA", CPS4$Country != "United States") %>% prop.table(1)
# 0.309
=======
table(CPS$MetroArea == "New York-Northern New Jersey-Long Island, NY-NJ-PA", CPS$Country != "United States")

area = "New York-Northern New Jersey-Long Island, NY-NJ-PA"
mean(CPS$Country[CPS$MetroArea==area] != "United States", na.rm=T)
#
#ans:0.3087
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

__§ 4.4__ Which metropolitan area has the largest number (note -- not proportion) of interviewees with a country of birth in India? 
```{r}
<<<<<<< HEAD
tapply(CPS4$Country == 'India', CPS4$MetroArea, sum, na.rm=TRUE) %>% sort %>% tail  
# New York-Northern New Jersey-Long Island, NY-NJ-PA : 96
=======
tapply(CPS$Country == 'India', CPS$MetroArea, sum, na.rm=T) %>% 
  sort %>% tail  
#
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

In Brazil?
```{r}
<<<<<<< HEAD
tapply(CPS4$Country == 'Brazil', CPS4$MetroArea, sum, na.rm=T) %>% sort %>% tail  
# Boston-Cambridge-Quincy, MA-NH : 18
=======
tapply(CPS$Country == 'Brazil', CPS$MetroArea, sum, na.rm=T) %>% 
  sort %>% tail  
#
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

In Somalia?
```{r}
<<<<<<< HEAD
tapply(CPS4$Country == 'Somalia', CPS4$MetroArea, sum, na.rm=T) %>% sort %>% tail 
# Minneapolis-St Paul-Bloomington, MN-WI : 17
=======
tapply(CPS$Country == 'Somalia', CPS$MetroArea, sum, na.rm=T) %>% 
  sort %>% tail  
#
#
>>>>>>> 29cb088a8bb7ef96191f373d87d43a8f8c70e434
```

<br><br><br>
